...
from utils.file_utils import edit_config_file
...
class CheckWorker(BaseWorker):
    ...
    def run(self):
        ...
        try:
            ...
            self._update_status(f"Ошибка проверки: {e}", level="ERROR")
            self._update_progress(0)
            self._update_text(f"Ошибка при проверке сервера:\n{traceback.format_exc()}")
...
class LaunchWorker(BaseWorker):
    ...
    def __init__(self, config, launch_data, parent=None):
        ...
        self._current_process = None # Ссылка на запущенный процесс BackOffice для остановки при отмене/ошибке
    ...
    def _stop_backoffice_process(self):
        """Останавливает запущенный процесс BackOffice, если он есть."""
        if self._current_process and self._current_process.poll() is None:
            ...
            self._current_process = None # Сбрасываем ссылку
    ...
    def run(self):
        ...
        try:
            ...
            if self._is_canceled: raise AbortOperation("Operation aborted.")
            ...
            self._update_progress(base + range_ * 0.1)
            ...
            if self.launch_data.get('app_info') is None:
                 ...
                 self._update_step_progress('process_response', 0.5)
            ...
            self._update_status(f"Ошибка запуска: {e}", level="ERROR")
            self._update_progress(0)
    ...
    def _run_from_step4(self):
        ...
        try:
            ...
            if not server_state_ok:
                 ...
                 self._update_step_progress('check_state', 0.5)
    ...
    def _run_from_step5(self):
        ...
        try:
            ...
            self.launch_data.update(step_data)
            self._current_process = self.launch_data.get('backoffice_process') # Сохраняем ссылку на процесс
            ...
            wait_success = step_wait_edit_config(
                self.config,
                self.launch_data,
                self._update_status,
                self._update_progress,
            ...
            if not wait_success:
                 # step_wait_edit_config вернет False, если отмена произошла во время wait_for_file
                 # Если это был таймаут, step_wait_edit_config выбросит TimeoutError
                 raise AbortOperation("Waiting for config file aborted.") # Это отмена, выбрасываем AbortOperation
            ...
            if not edit_config_file(
                ...
                self._update_status
                 ...
                 raise RuntimeError("Не удалось отредактировать файл конфигурации.")

            self._update_status("Файл конфигурации успешно отредактирован.")
            self._update_step_progress('wait_edit_config', 1.0)
            ...
            self.launch_data.update(step_data)
            self._current_process = self.launch_data.get('backoffice_process') # Сохраняем ссылку на новый процесс
            ...
            # --- Завершение ---
            self._update_status("Готово! BackOffice запущен с обновленной конфигурацией.", level="INFO")
            self._update_progress(100)

...
def parse_target_string(input_string):
    ...
    # Разделяем хост и порт. Ищем последнее двоеточие, за которым следуют цифры до конца строки или слэша.
    port_match = re.search(r":(\d+)(?:/.*)?$", temp_input)
    if port_match:
        try:
            port = int(port_match.group(1))
            logging.debug(f"Порт '{port}' извлечен из исходной строки.")
    ...
    logging.debug(f"Парсинг завершен: Хост/IP: '{url_or_ip}', Порт: {port}, IsIpAddress: {is_ip_address}")
